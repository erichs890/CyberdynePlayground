...
app.use((_req, res, next) => {
  ...
  res.json = (body) => { origJson(stripHints(body)); };
  next();
});
...
app.use('/', require('./routes/crawlable'));
app.use('/', require('./routes/html-vulns'));
...
app.get('/api', (_req, res) => {
  res.json({ name: 'Cyberdyne Systems API', version: '2.4.1', status: 'operational', docs: '/api/docs' });
});
